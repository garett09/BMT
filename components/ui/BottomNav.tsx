...
import { BarChart2, CreditCard, Home, Bell } from "lucide-react";
  ...
  return (
    <nav className="sticky bottom-0 border-t bg-background/70 backdrop-blur supports-[backdrop-filter]:bg-background/50">
      <div className="max-w-md mx-auto flex justify-around">
        {items.map((it) => {
          const icon = it.label === "Dashboard" ? <Home size={16} /> : it.label === "Transactions" ? <BarChart2 size={16} /> : it.label === "Accounts" ? <CreditCard size={16} /> : <Bell size={16} />;
          return (
