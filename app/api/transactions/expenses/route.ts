...
export async function POST(req: NextRequest) {
  ...
  const rl = await rateLimit(req, "transactions:expense:write", 60, 60);
  ...
  const tx: TransactionRecord = { id, userId, type: "expense", amount: parsed.data.amount, category: parsed.data.category, description: parsed.data.description, date: parsed.data.date ?? now.slice(0, 10), createdAt: now };
